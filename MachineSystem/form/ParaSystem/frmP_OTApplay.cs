...
** 描述：
**    主要用于加班类型信息的资料查询、删除、批量删除操作
*********************************************************************************/
namespace MachineSystem.TabPage
{
    public partial class frmP_OTApplay : Framework.Abstract.frmSearchBasic2
    {
        #region 变量定义
        /// <summary>
        /// 数据表
        ...
        // 日志
        private static readonly ILog log = LogManager.GetLogger(typeof(frmP_OTApplay));
        #endregion

        #region 画面初始化

        public frmP_OTApplay()
        {
            InitializeComponent();
        ...
        protected override void SetInsertInit(bool isClear)
        {
            ...
            try
            {
                frmEditP_OTApplay frm = new frmEditP_OTApplay();
                ...
                if (XtraMsgBox.Show("是否删除数据？", this.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                {
                    ...
                    if (drs.Length > 0)
                    {
                        for (int i = 0; i < drs.Length; i++)
                        {
                            ...
                            {
                                //日志
                                SysParam.m_daoCommon.WriteLog("免许类型设置:" ,"新增", dr["OTApply"].ToString());
                            }
                        }
                    }
                    if (result > 0)
        ...
        protected override void GetDspDataList()
        {
            try
            {
                ...
                string str_sql = "Select  CAST('0' AS Bit) AS SlctValue,* From P_OTApply ";

                if (txtpName.Text.Trim() != "")
                {
                    str_sql += " where ";
                    str_sql += " OTApply like '%" + txtpName.Text.Trim() + "%' ";
                }
                str_sql += " Order by IndexNum asc  ";
                ...
                gridView1.Columns["IndexNum"].OptionsColumn.ReadOnly = true;
                gridView1.Columns["IndexNum"].OptionsColumn.AllowEdit = false;
                gridView1.Columns["OTApply"].OptionsColumn.ReadOnly = true;
                gridView1.Columns["OTApply"].OptionsColumn.AllowEdit = false;
                gridView1.Columns["OTKind"].OptionsColumn.ReadOnly = true;
                gridView1.Columns["OTKind"].OptionsColumn.AllowEdit = false;
                gridView1.Columns["Remark"].OptionsColumn.ReadOnly = true;
                gridView1.Columns["Remark"].OptionsColumn.AllowEdit = false;
