...
** 描述：
**    主要用于 离职原因设置 信息的资料查询、删除、批量删除操作
*********************************************************************************/
namespace MachineSystem.TabPage
{
    public partial class frmP_LeaveReason : Framework.Abstract.frmSearchBasic2
    {
        #region 变量定义
        /// <summary>
        /// 数据表
        ...
        // 日志
        private static readonly ILog log = LogManager.GetLogger(typeof(frmP_LeaveReason));
        #endregion

        #region 画面初始化

        public frmP_LeaveReason()
        {
            InitializeComponent();
        ...
        protected override void SetInsertInit(bool isClear)
        {
            ...
            try
            {
                frmEditP_LeaveReason frm = new frmEditP_LeaveReason();
                ...
                if (XtraMsgBox.Show("是否删除数据？", this.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                {
                    ...
                    if (drs.Length > 0)
                    {
                        for (int i = 0; i < drs.Length; i++)
                        {
                            ...
                            {
                                //日志
                                SysParam.m_daoCommon.WriteLog("离职原因", "删除",  dr["pName"].ToString());
                            }
                        }
                    }
                    if (result > 0)
