...
** 描述：
**    主要用于 免许资格 设置 信息的资料查询、删除、批量删除操作
*********************************************************************************/
namespace MachineSystem.TabPage
{
    public partial class frmP_LicenseEntitle : Framework.Abstract.frmSearchBasic2
    {
        #region 变量定义
        ...
        // 日志
        private static readonly ILog log = LogManager.GetLogger(typeof(frmP_LicenseEntitle));
        #endregion
        ...
        protected override void SetInsertInit(bool isClear)
        {
            ...
            try
            {
                frmEditP_LicenseEntitle frm = new frmEditP_LicenseEntitle();
                ...
                if (XtraMsgBox.Show("是否删除数据？", this.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                {
                    ...
                    if (drs.Length > 0)
                    {
                        for (int i = 0; i < drs.Length; i++)
                        {
                            ...
                            {
                                //日志
                                SysParam.m_daoCommon.WriteLog("免许资格设置:", "删除", dr["pName"].ToString());
                            }
                        }
                    }
                    if (result > 0)
        ...
        protected override void GetDspDataList()
        {
            try 
            {
                txtpMark.Enabled = true;
                txtpMark.Properties.ReadOnly = false;
                ...
                if (txtpName.Text.Trim() != "" || txtpMark.Text.Trim() != "")
                {
                    str_sql += " where ";
                    if (txtpName.Text.Trim() != "") 
                    {
                        str_sql += "pName like '%" + txtpName.Text.Trim() + "%' and ";
                    }
                    if (txtpMark.Text.Trim() != "")
                    {
                        str_sql += "pMark like '%" + txtpMark.Text.Trim() + "%' and ";
                    }
                    str_sql = str_sql.Substring(0, str_sql.Length - 4);
                }
