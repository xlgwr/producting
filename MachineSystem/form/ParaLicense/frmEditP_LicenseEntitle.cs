...
** 描述：
**    主要用于免许类型 信息的资料新增、修改操作
*********************************************************************************/
namespace MachineSystem.form.ParaLicense
{
    public partial class frmEditP_LicenseEntitle : Framework.Abstract.frmBaseToolEntryXC//frmSearchBasic2
    {
        #region 变量定义
        ...
        public DataRow dr;
        // 日志
        private static readonly ILog log = LogManager.GetLogger(typeof(frmEditP_LicenseEntitle));
        #endregion
        ...
        /// 构造函数
        /// </summary>
        public frmEditP_LicenseEntitle()
        {
            InitializeComponent();
            this.TableName = "P_License_Entitle";//操作表名称
        }
        ...
        protected override void SetInsertProc(ref int RtnValue)
        {
            ...
            try
            {
                ...
                m_dicItemData["pMark"] = txtpMark.Text.Trim();

                int result = SysParam.m_daoCommon.SetInsertDataItem("P_License_Entitle", m_dicItemData);
                if (result > 0)
                {
                    XtraMsgBox.Show("新增数据成功！", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    //日志
                    SysParam.m_daoCommon.WriteLog("资格设置", "新增", txtpName.Text.Trim());
        ...
        public override void SetModifyProc(ref int RtnValue)
        {
            ...
            try
            {
                ...
                int result = SysParam.m_daoCommon.SetModifyDataItem("P_License_Entitle", m_dicItemData, m_dicPrimarName);
                if (result > 0)
                {
                    XtraMsgBox.Show("修改数据成功！", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    //日志
                    SysParam.m_daoCommon.WriteLog("资格设置", "修改", txtpName.Text.Trim());
                    DialogResult = DialogResult.OK;
                }
            }
            catch (Exception ex)
        ...
        protected override void GetInputCheck(ref bool isSucces)
        {
            try
            {
                ...
                if (isSucces)
                {
                    if (string.IsNullOrEmpty(this.txtpName.Text.Trim()))
                    {
                        isSucces = false;
        ...
        public void GetDataRowValue(DataRow dr)
        {
            ...
            txtpMark.Text = dr["pMark"].ToString();
        }
        #endregion
    }
}
