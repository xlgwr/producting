...
** 描述：
**    主要用于人员状态信息的资料新增、修改操作
*********************************************************************************/
namespace MachineSystem.TabPage
{
    public partial class frmEditP_Produce_User_Status1 : Framework.Abstract.frmBaseToolEntryXC
    {
        #region 变量定义
        public DataRow dr;
        // 日志
        private static readonly ILog log = LogManager.GetLogger(typeof(frmEditP_Produce_User_Status1));
        #endregion

        #region 画面初始化

        public frmEditP_Produce_User_Status1()
        {
            InitializeComponent();
        ...
        protected override void SetInsertProc(ref int RtnValue)
        {
            ...
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    //日志
                    SysParam.m_daoCommon.WriteLog("用户状态设置", "新增", txtpName.Text.Trim());
        ...
        public override void SetModifyProc(ref int RtnValue)
        {
            ...
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    //日志
                    SysParam.m_daoCommon.WriteLog("用户状态设置：", "修改", txtpName.Text.Trim());
                    DialogResult = DialogResult.OK;
                }
            }
            catch (Exception ex)
