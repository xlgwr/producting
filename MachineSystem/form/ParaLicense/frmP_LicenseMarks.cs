...
** 描述：
**    主要用于免许等级 设置 信息的资料查询、删除、批量删除操作
*********************************************************************************/
namespace MachineSystem.TabPage
{
    public partial class frmP_LicenseMarks : Framework.Abstract.frmSearchBasic2
    {
        #region 变量定义
        ...
        // 日志
        private static readonly ILog log = LogManager.GetLogger(typeof(frmP_LicenseMarks));
        #endregion
        ...
        protected override void SetInsertInit(bool isClear)
        {
            ...
            try
            {
                frmEditP_LicenseMarks frm = new frmEditP_LicenseMarks();
                ...
                if (XtraMsgBox.Show("是否删除数据？", this.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                {
                    ...
                    if (drs.Length > 0)
                    {
                        for (int i = 0; i < drs.Length; i++)
                        {
                            ...
                            {
                                //日志
                                SysParam.m_daoCommon.WriteLog("免许等级设置:", "删除", dr["pName"].ToString());
                            }
                        }
                    }
                    if (result > 0)
