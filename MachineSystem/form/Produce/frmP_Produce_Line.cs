...
** 描述：
**    主要用于line别 信息的资料查询、删除、批量删除操作
*********************************************************************************/
namespace MachineSystem.TabPage
{
    public partial class frmP_Produce_Line : Framework.Abstract.frmSearchBasic2
    {
        #region 变量定义
        /// <summary>
        /// 数据表
        ...
        // 日志
        private static readonly ILog log = LogManager.GetLogger(typeof(frmP_Produce_Line));
        #endregion

        #region 画面初始化
        public frmP_Produce_Line()
        {
            InitializeComponent();
            this.m_GridViewUtil.GridControlList = this.gridControl1;
            this.m_GridViewUtil.ParentGridView = this.gridView1;
            this.TableName = "P_Produce_Line";//操作表名称
        ...
        protected override void SetInsertInit(bool isClear)
        {
            ...
            try
            {
                frmEditP_Produce_Line frm = new frmEditP_Produce_Line();
                ...
                if (XtraMsgBox.Show("是否删除数据？", this.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                {
                    ...
                    if (drs.Length > 0)
                    {
                        for (int i = 0; i < drs.Length; i++)
                        {
                            ...
                            {
                                //日志
                                SysParam.m_daoCommon.WriteLog("line别:", "删除", dr["pName"].ToString());
                            }
                        }
                    }
                    if (result > 0)
            ...
            try
            {
                if (drs.Length > 0)
                {
                    for (int i = 0; i < drs.Length; i++)
                    {
                        ...
                        str_sql += " and LineID= " + dr["ID"].ToString();

                        m_tblDataList = SysParam.m_daoCommon.GetTableInfoBySqlNoWhere(str_sql);
                        if (m_tblDataList.Rows.Count > 0)
                        {

                            XtraMsgBox.Show("因为Line别是："
                                            + dr["pName"].ToString() + "  "
                                            + " 已经有关联数据，不能删除！",
