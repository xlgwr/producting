...
{
    public partial class frmP_Produce_Guanwei : Framework.Abstract.frmSearchBasic2
    {
        #region 变量定义
        /// <summary>
        /// 数据表
        ...
        // 日志
        private static readonly ILog log = LogManager.GetLogger(typeof(frmP_Produce_Guanwei));
        #endregion

        #region 画面初始化

        public frmP_Produce_Guanwei()
        {
            InitializeComponent();
        ...
        protected override void SetInsertInit(bool isClear)
        {
            ...
            try
            {
                frmEditP_Produce_Guanwei frm = new frmEditP_Produce_Guanwei();
                ...
                if (XtraMsgBox.Show("是否删除数据？", this.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                {
                    ...
                    if (drs.Length > 0)
                    {
                        for (int i = 0; i < drs.Length; i++)
                        {
                            ...
                            {
                                //日志
                                SysParam.m_daoCommon.WriteLog("关位名称设置:", "删除", dr["pName"].ToString());
                            }
                        }
                    }
                    if (result > 0)
        ...
        protected override void GetDspDataList()
        {
            try
            {
                ...
                gridView1.Columns["pType"].OptionsColumn.ReadOnly = true;
                gridView1.Columns["pType"].OptionsColumn.AllowEdit = false;
            }
            catch (Exception ex)
            {
                log.Error(ex);
            ...
            try
            {
                if (drs.Length > 0)
                {
                    for (int i = 0; i < drs.Length; i++)
                    {
                        ...
                        str_sql += " and GuanweiID= " + dr["ID"].ToString();

                        m_tblDataList = SysParam.m_daoCommon.GetTableInfoBySqlNoWhere(str_sql);
                        if (m_tblDataList.Rows.Count > 0)
                        {

                            XtraMsgBox.Show("因为关位是："
                                            + dr["pName"].ToString() + "  "
                                            + " 已经有关联数据，不能删除！",
