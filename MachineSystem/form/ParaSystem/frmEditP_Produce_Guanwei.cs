...
{
    public partial class frmEditP_Produce_Guanwei :Framework.Abstract.frmBaseToolEntryXC
    {
        #region 变量定义
        public DataRow dr;
        // 日志
        private static readonly ILog log = LogManager.GetLogger(typeof(frmEditP_Produce_Guanwei));
        #endregion

        #region 画面初始化

        public frmEditP_Produce_Guanwei()
        {
            InitializeComponent();
            this.TableName = "P_Produce_Guanwei";//操作表名称
        }
        ...
        protected override void SetInsertProc(ref int RtnValue)
        {
            ...
            try
            {
                ...
                m_dicItemData["pType"] = comboBoxGuanweiType.Text ;
                ...
                if (result > 0)
                {
                    ...
                    //日志
                    SysParam.m_daoCommon.WriteLog("关位名称设置", "新增", txtpName.Text.Trim() + "类型：" + comboBoxGuanweiType.Text);
        ...
        public override void SetModifyProc(ref int RtnValue)
        {
            ...
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    //日志
                    SysParam.m_daoCommon.WriteLog("关位名称设置", "修改", txtpName.Text.Trim() + "类型：" + comboBoxGuanweiType.Text);
                    DialogResult = DialogResult.OK;
                }
            }
            catch (Exception ex)
        ...
        protected override void GetInputCheck(ref bool isSucces)
        {
            try
            {
                ...
                if (isSucces)
                {
                    ...
                    {
                        if (Encoding.Default.GetByteCount(this.txtpName.Text.Trim())> 40)
                        {
                            isSucces = false;
        ...
        public void GetDataRowValue(DataRow dr)
        {
            ...
            comboBoxGuanweiType.Text = dr["pType"].ToString(); ;
        }
