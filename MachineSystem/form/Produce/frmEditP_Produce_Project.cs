...
** 描述：
**    主要用于工程别设置信息的资料新增、修改操作
*********************************************************************************/
namespace MachineSystem.TabPage
{
    public partial class frmEditP_Produce_Project : Framework.Abstract.frmBaseToolEntryXC
    {
        
        #region 变量定义
        public DataRow dr;
        // 日志
        private static readonly ILog log = LogManager.GetLogger(typeof(frmEditP_Produce_Project));

        #endregion

        #region 画面初始化
        public frmEditP_Produce_Project()
        {
            InitializeComponent();
        ...
        protected override void SetInsertProc(ref int RtnValue)
        {
            ...
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    //日志
                    SysParam.m_daoCommon.WriteLog("申请事由设置", "新增", txtName.Text.Trim());
        ...
        public override void SetModifyProc(ref int RtnValue)
        {
            ...
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    //日志
                    SysParam.m_daoCommon.WriteLog("加班类型设置：", "修改", txtName.Text.Trim());
                    DialogResult = DialogResult.OK;
                }
            }
            catch (Exception ex)
